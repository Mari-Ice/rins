...
from task3.msg import AnomalyInfo
...
def fit_pca(matrix, n_components=150):
...
class face_extractor(Node):
	def __init__(self):
		...
		self.anomaly_pub = self.create_publisher(AnomalyInfo, "/anomaly_info", QoSReliabilityPolicy.BEST_EFFORT)
		...
		self.pca = fit_pca(matrix, n_components=150)
	...
	def rgb_callback(self, rgb_data):
		...
		for x in res:
			...
			if(min_err < 0.5 and min_err >= 0):
				...
				self.mona_img = masked[int(cbox[1]):int(cbox[3]), int(cbox[0]):int(cbox[2])]
	...
	def detect_anomalies(self):
		if self.mona_img is None or self.fresh is False:
			return

		mona_img = self.mona_img
		...
		print("max: ", np.max(err[self.margin:-self.margin, self.margin:-self.margin]))

		# threshold the error image
		err[err < 0.05] = 0
		err[err >= 0.05] = 1

		# print the number of pixels in the error, ignoring the margin
		q = np.sum(err[self.margin:-self.margin, self.margin:-self.margin])
		print(q)
		...
		if (np.sum(err[self.margin:-self.margin, self.margin:-self.margin]) > 10):
			msg = AnomalyInfo()
			msg.quality = q
